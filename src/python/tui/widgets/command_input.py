...
class CommandInput(Input):
    ...
    SEGMENT_PLACEHOLDER = "[SEGMENT] 1-0/qw play | ←→ nudge | []=marker | Space=play | i=insert"

    def _update_placeholder(self) -> None:
        """Update placeholder based on current mode."""
        if self._mode == self.MODE_SEGMENT:
            self.placeholder = self.SEGMENT_PLACEHOLDER
        else:
    ...
    def on_key(self, event) -> None:
        ...
        if self._mode == self.MODE_SEGMENT:
            ...
            # Up/Down arrows - route through MarkerNudge (app scrolls the output panel)
            if key == "up":
