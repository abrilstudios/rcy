...
def ep133_upload_bank(args, app: 'RCYApp') -> str:
    ...
    for i in range(seg_count):
        ...
        # Convert to mono (EP-133 prefers mono for samples)
        if np.array_equal(segment_left, segment_right):
            segment_data = segment_left
        else:
            segment_data = (segment_left + segment_right) / 2
        ...
        try:
            ...
            _device.assign_sound(node_id, target_slot)
            assigned += 1
    ...
    return f"Uploaded {uploaded} segments to {project}/{bank} (slots {slot_start}-{slot_end}, pads 1-{seg_count})"
